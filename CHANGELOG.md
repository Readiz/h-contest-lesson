...
- 레슨 시각 자료를 다시 검토해 정적 SVG 8개를 개선하고 CCW/선분 교차 도식 1개를 추가했습니다. Beam 연결 오류와 score/key 표기를 교정했습니다.
- 이분 탐색·BFS·Dijkstra·2차원 누적합·열린 경로 2-opt·SA에 입력을 바꾸고 이전/다음/재생으로 따라갈 수 있는 독립 데모 6개를 추가했습니다.
- 그림과 데모는 관련 설명 옆에 배치하고, 실제 모델·독립 기준값 검증과 모바일 가독성 기준을 기여 절차에 추가했습니다. 98개 레슨·168개 본문 구성은 유지합니다.

## 2026-09-22 STL 없는 풀이 흐름과 공통 라이브러리

- `실전 C++ 제출과 검증`에서 ORDERING → 검증 → SA로 이어지는 흐름을 유지하고, 배열·난수·정렬·큐·힙을 별도 `STL 없는 공통 라이브러리`로 분리했습니다. 필요한 블록만 원문에서 추출하는 도구를 추가했습니다.
- 정렬·그리디·힙 예제를 공통 블록과 조합하는 STL 없는 함수로 바꾸고, 입력 상한·초기화·동률·용량 계약을 맞췄습니다. 다른 123개 본문은 공통 코드 링크를 갱신했습니다.
- 98개 강의(기본 31·참고 67), 등록 본문 168개, C++ 블록 230개입니다. 실제 본문 조합과 독립 기준 풀이 검증을 전체 validator에 추가했습니다. 상세 범위와 결과는 [검토 기록](LESSON_REVIEW.md)에 남겼습니다.
...
- 기본 트랙의 선수 순서를 맞추고 C++ 문서 130개의 적용 환경을 표시했습니다. 도입부에 몰린 조건은 관련 개념·코드 옆으로 옮겼습니다.
- ORDERING SA에 실행 코드·지역 최적 반례·실측을 추가하고, 연습이 없던 기본 강의 19개를 완결형 로컬 연습으로 보강했습니다. 기본 30개 모두 실제 연습을 제공합니다.
- 강의 97개·본문 169개·기존 C++ 228개 블록은 유지합니다. 추가 검증과 측정 조건은 [검토 기록](LESSON_REVIEW.md)에 남겼습니다.

## 2026-09-09 공개 탐색 지도 수정

공개 카탈로그 학습 지도 두 곳의 삭제된 강의 ID를 Monge와 SMAWK로 교체했습니다. 알 수 없는 강의 ID는 일반 텍스트로 숨기지 않고 카탈로그 생성 오류로 처리합니다.

## 2026-09-09 — 유지 자료 재검토

- 기본자료 유지 항목 10개·본문 18개 재검토. TC 초기화, 열린 경로 꼬리, SA 상태, 배치 repair, 블록 lazy, DP 경로 분할 그림 6개 추가.
- 스트레스 테스트에 잘못된 사이클 차분식 반례, 검수 게이트 교환 증명에 누락 경우, SA 동률 이동 처리 보완.
- 강의 97개·본문 169개 유지.

## 2026-09-09 기본자료 후속 시각화

기본자료 후속 후보 12개에 알고리즘 구조·상태 변화 SVG와 확대 링크를 추가했습니다. 기존 8개와 합쳐 이번 전수 검토 후 총 20개 자료에 그림을 보강했습니다. 앱에서 무효 처리되던 상대 SVG 확대 링크도 20개 모두 공개 원본 URL로 수정했습니다. 코드와 카탈로그 개수는 유지하며 [검토 기록](LESSON_REVIEW.md)의 우선순위 표를 갱신했습니다.

## 2026-09-09 전체 190개 재검토와 기본자료 그림

등록 본문 190개를 모두 읽고 직접 이어지는 설명·실습 21개 경계를 통합해 169개로 정리했습니다. 강의 97개(기본 30/참고 67)는 유지합니다. SAM 중복 구현, MDP와 penalty 모델 흐름을 정리하고 설명·경계 오류를 수정했습니다. Manacher 완성 구현과 전수 소형 검증, 기본자료 SVG 8개를 추가했습니다. 세부 문서별 판단과 30개 자료의 시각화 우선순위는 [전체 검토 기록](LESSON_REVIEW.md)에 있습니다.

이 문서는 이미 공개된 레슨의 이동 기록과 콘텐츠 보강 완료 내역을 보관합니다. 아직 공개하지 않은 후보와 practice link TODO는 [ROADMAP.md](ROADMAP.md)에서 관리합니다.

## 2026-09-09 실제 렌더링 확인 후 CHT 설명 통합

- 운영 게시본을 읽는 격리 로컬 UI에서 기본 30·참고 67·전체 97개 표시와 CHT 응용 페이지를 확인했습니다. 운영 URL은 로그인 화면까지만 확인했으며 authenticated 본문 검증과 구분합니다.
- CHT 기본·변형·DP 적용 3개 페이지를 식 전개 → 단조 deque → 실제 DP와 손계산 → Li Chao 순서로 통합했습니다. 기울기가 증가한다는 잘못된 설명을 비증가로 고쳤고, 같은 기울기·같은 x 허용 조건을 구현 옆에 뒀습니다.
- C++ 구현 3개를 그대로 보존하고 검증 스니펫 순서·내부 링크·카탈로그를 동기화했습니다. 강의 97개는 유지하며 하위 페이지는 95개→93개, 전체 본문은 192개→190개입니다.

## 2026-09-09 기본·응용 카드와 짧은 개요의 본문 통합

- XOR Linear Basis와 Rotating Calipers의 응용 카드를 기본 강의로 합쳤습니다. 기저 정규화·k번째 XOR 및 최소 폭 구현을 기본 정의 바로 뒤에서 읽으며, calipers는 동일한 Point·외적·거리 함수를 사용합니다.
- DP는 상태·배열 갱신·트리 DP를, Segment Tree는 구간 질의·lazy 갱신을, TSP는 완전탐색·DP·휴리스틱을 한 본문으로 연결했습니다. TSP 이미지의 상대 경로도 이동 위치에 맞췄습니다.
- FPS log·exp는 기본 연산 본문으로, 점화식 추정·holdout 검증은 BM 구현 본문으로 합쳤습니다. 10개 문서 경계를 없애 공개 강의 99개→97개, 본문 202개→192개로 정리했습니다. 서로 다른 모델을 비교하는 Game Theory Applications와 독립적인 알고리즘 페이지는 유지합니다.
- manifest의 선수·후속·관련 강의, 카탈로그 생성 경로, 기존 이동 안내와 검증 스니펫 경로를 갱신했습니다. calipers 공유 코드 검산과 BM 계수·검증 연결 검사를 추가했습니다.

## 2026-09-09 설명 흐름을 끊는 페이지 경계 통합

- Treap 개요·핵심 연산, 선형대수 선택·연습, Gomory-Hu 구성·손계산, Ukkonen 상태·phase trace, Suffix Array 기본·응용, planar face 순회·dual 구성을 각각 같은 본문으로 합쳤습니다. 6개 페이지를 줄이고 고유 코드·예시·연습을 보존했습니다.
- Suffix Array의 반복 소개·응용표, 선형대수의 중복 parity 예시, planar dual의 중복 Euler 설명을 정리했습니다. face 순회 결과를 dual 구성 함수에 넣는 연결도 명시했습니다.
- CONTRIBUTING의 길이·개념 개수 기준을 독립 학습 질문 기준으로 바꾸고 manifest·내부 링크·생성 카탈로그를 동기화했습니다.
...
- Versioning Model Map과 Suffix Model Map의 선택 기준·메모리·인덱스 주의점을 각 허브 본문에 통합했습니다. 참고 본문은 184개에서 182개가 됐고 manifest와 생성 카탈로그를 함께 갱신했습니다.
- Matroid Intersection의 항상 참을 반환하는 미완성 oracle 예제를 제거하고, 구현해야 할 독립성 판정 계약을 설명했습니다.
- 실제 연습이 없는 강의 47개의 `linked` 표시를 `todo`로 바로잡았습니다. 두 노트 모두 본문 TODO와 근거 없는 연습 표시를 validator에서 검사하며, 로컬 연습 제목도 인식합니다.

## 2026-09-08 풀이 예시와 중복 구현 정리

- 기본 30개 강의를 다시 검토해 자기 자신을 호출하는 stress 비교, 핵심 전이가 없는 Digit DP·구간 DP, 임의 점수만 더하는 Beam 골격, 미리 만들어진 게임 트리 래퍼를 제거했습니다. 테스트 강의는 TSP의 독립 기준 답과 ORDERING의 전체 비용·차분·복구 검증으로 연결했습니다.
- 부분 구현 뒤에 같은 전체 코드를 반복하거나 다른 강의의 DFS·DSU·힙을 복제한 부분을 정리했습니다. C++ 블록은 240개에서 87개로 줄었고, 공통 스니펫 7개와 ORDERING·Hungarian의 제출 구현은 유지했습니다.
- 하위 페이지 11개를 삭제·통합해 기본 트랙의 Markdown 본문은 58개에서 47개가 됐습니다. 트리 DP는 상태·전이 페이지로, TSP 복원·메모리는 DP 옆으로, Segment Tree의 집계·lazy 합성은 해당 구현 옆으로, Treap 순위 연산은 핵심 연산 페이지로 옮겼습니다.
- Hungarian 손계산은 같은 alternating 탐색을 목록·ASCII·표·그림으로 반복하던 부분을 줄이고, line cover와 delta 조정의 근거를 남겼습니다. 실제 문제 사례와 서로 다른 조건을 설명하는 반례는 보존했습니다.
- manifest의 페이지·소개·태그·예상 시간·구현 완성도를 남은 내용에 맞추고, 기여 지침과 템플릿에 예시의 설명 목적과 중복 구현 금지 기준을 추가했습니다.

## 2026-09-08 기본 30개 강의 문장과 흐름 정리

- `heuristic-notes` 30개 강의와 하위 페이지 전체에서 반복 결론, 같은 내용을 바꿔 쓴 실수 표·체크리스트, 중복 학습 안내를 걷어냈습니다. 경계·초기화·정당성 조건은 해당 설명과 구현 옆에 남겼습니다.
- 본문 59개 문서 중 TODO만 있던 DP 연습 페이지 하나를 삭제했습니다. 연습 예정 99개 행은 ROADMAP의 강의별 후보 목록으로 옮기고, 실제 문제는 필요한 본문이나 짧은 실습으로 연결했습니다. 누적합·좌표 압축 등 6개 강의의 연습 상태를 실제 내용에 맞게 `todo`로 바로잡았습니다.
- 코드·텍스트 예시를 포함한 원문은 약 18%, 코드 블록을 제외한 설명은 약 29% 줄었습니다. C++ 코드 블록 240개는 내용 변경 없이 보존했습니다.
- Hungarian의 배송 예시에서 copy 번호 순서 배정을 greedy로 잘못 부르던 설명을 고쳤고, 비용 행렬의 최소 합과 실제 운행 점수의 차이를 명시했습니다.
- 작성 지침·템플릿·PR 양식에서 고정 결론과 네 단계 연습표를 요구하던 규칙을 제거했습니다. validator는 기본 트랙의 TODO 노출과 잘못된 연습 상태를 검사하며, 연습 절이나 표 형식은 강제하지 않습니다. 참고 70개 강의의 본문은 이번 편집 범위에 포함하지 않았습니다.

## 2026-09 실전 풀이 중심 개편 시작

- 기존 `cpp-contest-basics`를 `실전 C++ 기본기와 공통 코드`로 개편했습니다. 함수 구현형 제출 계약, TC 초기화, 배열·난수·정렬·큐·힙, 로컬 검증과 ORDERING 기준선을 4개 페이지로 연결했습니다.
- 각 공통 코드 블록의 용량·인덱스·초기화·실패 조건을 설명하고, 원문 블록 7개를 추출해 경계·참조 구현 대조·ASan/UBSan 검사를 실행하는 `scripts/check_cpp_basics.py`를 추가했습니다. 레슨 validator와 CI에서 함께 실행합니다.
- 처음 문제를 푸는 경로를 공통 코드 → ORDERING → 모델링으로 안내하고, 이후 실험·SA·빔·배정/경로·배치 개편을 ROADMAP에 남겼습니다. 전체 레슨 순서와 기존 STL 예제의 일괄 전환은 후속 단계입니다.
- h-contest의 강의 API는 로그인 사용자에게 제공됩니다. 원본 저장소와 Pages의 공개 범위는 별도 정책입니다.
...
- 참고 노트로 이동한 고급 레슨은 `matching-cover-duality`, `dominator-tree`, `weighted-matching`, `directed-mst`, `versioned-data-structures`, `wavelet-tree`, `wavelet-matrix`, `succinct-bitvector`, `link-cut-tree`, `suffix-periodicity-structures`, `palindrome-structures`, `lyndon-factorization`, `line-arrangement`, `voronoi-delaunay`, `polynomial-recurrence-algorithms`, `dirichlet-convolution`, `multiplicative-functions`, `summatory-number-theory`, `geometry-robustness-and-duality`, `black-box-linear-algebra`, `sparse-linear-systems`, `linear-algebra-applications`, `online-convex-optimization`, `randomized-determinant`, `matrix-tree-theorem-applications`, `planar-graph-duality`입니다.
- 기본 및 심화 노트를 현재 문제 풀이에 바로 쓰는 32개 직접 학습 트랙으로 더 좁히고, 나머지 66개 레슨은 참고 노트로 재분류했습니다.
- 수학/기하/최적화 성격의 `modular-arithmetic`, `geometry-ccw-segment-intersection`, `gcd-extended-euclid-crt`, `combinatorics-ncr`, `probability-expected-value`, `parametric-optimization`도 참고 노트로 옮겨 직접 학습 트랙을 26개로 좁혔습니다.
...
- `online-convex-optimization` Practice Set에 full-information expert advice와 Multiplicative Weights 로컬 구현 연습을 추가했습니다.
...
- `matroid-algorithms` Practice Set에 weighted partition matroid greedy 로컬 연습과 exchange argument를 추가했습니다.
...
- `planar-graph-duality` Practice Set에 face incidence 기반 dual shortest path 로컬 구현 연습을 추가했습니다.
...
- `convex-dp-optimization` Practice Set에 line-query DP trace와 Li Chao Tree 로컬 구현 연습을 추가했습니다.
...
- `probabilistic-decision-ai` Practice Set에 finite-horizon MDP Bellman DP trace와 로컬 구현 연습을 추가했습니다.
...
- `geometry-robustness-and-duality` Practice Set에 `__int128` orientation 기반 exact segment intersection 로컬 연습을 추가했습니다.
