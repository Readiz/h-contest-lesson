...
| 전략과 최적화 | 그리디, 동적 계획법, 휴리스틱처럼 풀이 방향을 정하는 사고 도구입니다. | 14 |
| 그래프와 트리 | 탐색, 최단거리, DAG, 트리 구조를 다루는 그래프 계열 개념입니다. | 21 |
| 자료구조 | 우선순위 큐, Union-Find, 구간 자료구조, 균형 트리 계열을 모았습니다. | 17 |
| 문자열 | 패턴 매칭, 해싱, Trie, suffix 구조처럼 문자열을 빠르게 비교하고 탐색하는 개념입니다. | 5 |
| 기하 | CCW, 선분 교차, 볼록 껍질처럼 좌표와 벡터를 다루는 기하 기본 개념입니다. | 14 |
| 수학 | 모듈러 연산, 정수론, 조합론처럼 경우의 수와 수식 처리에 필요한 개념입니다. | 20 |
