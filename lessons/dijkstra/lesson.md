...
| 단계 | 문제 | 목표 |
| --- | --- | --- |
| 입문 | 모든 간선 비용이 1인 최단거리 | BFS와 Dijkstra를 비교하며 거리 배열 의미 확인 |
| 표준 | 양수 가중치 그래프의 단일 시작점 최단거리 | 우선순위 큐, 오래된 후보 제거, `long long` 거리 구현 |
| 응용 | 여러 시작점 또는 목표 정점 하나 | multi-source 초기화와 조기 종료 조건 적용 |
| 함정 | 음수 간선이 섞인 그래프 판별 | Dijkstra를 쓰면 안 되는 조건과 Bellman-Ford 필요성 확인 |
