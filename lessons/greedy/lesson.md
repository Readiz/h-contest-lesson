...
그리디는 복잡도보다 정당성 증명이 더 중요합니다. 빠른 기준이라도 반례가 있으면 사용할 수 없습니다.

## 13. 자주 하는 실수

- 그럴듯한 정렬 기준을 세우고 반례를 확인하지 않습니다.
- "가장 큰 값"과 "가장 작은 마감"처럼 서로 다른 기준을 섞어 정당성을 잃습니다.
- 동률 처리 기준이 답의 안정성에 영향을 주는데 comparator에 넣지 않습니다.
- 최적해를 증명해야 하는 문제를 휴리스틱 점수 개선 문제처럼 풉니다.

## 14. 문제를 볼 때 체크할 조건

1. 선택을 한 번 확정하면 나중에 되돌리지 않아도 되는가?
2. 정렬 기준이나 우선순위 기준이 명확한가?
3. 작은 반례를 직접 만들었을 때 기준이 깨지지 않는가?
4. 항상 앞서기, 교환 논증, 증거 논증 중 하나로 설명할 수 있는가?
5. 후보가 시간에 따라 추가되면 우선순위 큐가 필요한가?

## 15. 연습 문제

| 단계 | 문제 | 목표 | 힌트 키워드 |
| --- | --- | --- | --- |
| 입문 | TODO: 회의실 배정 문제 추가 | 끝나는 시간이 빠른 순서의 정당성 확인 | interval scheduling |
| 표준 | [검수 게이트 예약](/practice/INSPECTION) | 가능한 시간 구간에 요청을 최대한 배정 | deadline, greedy |
| 응용 | TODO: 마감이 있는 과제 선택 문제 추가 | 정렬 + 우선순위 큐 패턴 적용 | deadline, priority queue |
| 함정 | TODO: 그리디 반례 찾기 문제 추가 | 그럴듯한 기준이 깨지는 작은 입력 만들기 | counterexample |
