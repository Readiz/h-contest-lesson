...
| 단계 | 문제 | 목표 | 힌트 키워드 |
| --- | --- | --- | --- |
| 입문 | TODO: 두 수의 합 찾기 문제 추가 | 정렬 후 양끝 포인터를 그대로 적용 | 양끝 포인터 |
| 표준 | TODO: 합이 S 이상인 가장 짧은 양수 구간 문제 추가 | `while (sum >= S)` 위치 익히기 | sliding window |
| 응용 | TODO: 서로 다른 값이 K개 이하인 가장 긴 구간 문제 추가 | 빈도 map과 가변 길이 창 결합 | frequency map |
| 함정 | TODO: 음수가 섞인 구간 합 문제 추가 | 투 포인터가 깨지는 조건 확인 | 단조성 깨짐 |
