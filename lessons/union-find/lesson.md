...
이미 같은 집합인 두 원소에 대해 `unite`를 호출하면 컴포넌트 개수는 줄어들면 안 됩니다. 성공한 `unite`만 세면 이를 구분할 수 있습니다.
...
집합을 합친 뒤에는 예전에 대표였던 노드가 지금도 대표라고 가정하면 안 됩니다. 대표 기준 정보를 읽을 때는 항상 `find`를 한 번 거치는 편이 안전합니다.
