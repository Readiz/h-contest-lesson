...
| 단계 | 문제 | 목표 | 힌트 키워드 |
| --- | --- | --- | --- |
| 입문 | [구간 합 함수](/practice/FUNCSUM1) | 큰 구간 합을 반복 없이 계산하고 overflow 확인 | arithmetic sum, `long long` |
| 표준 | TODO: 여러 구간 합 질의 문제 추가 | prefix 배열을 `n + 1`칸으로 두는 패턴 익히기 | prefix sum |
| 응용 | TODO: 여러 구간 업데이트 후 최종 배열 문제 추가 | 차분 배열로 업데이트를 모아 한 번에 반영 | difference array |
| 함정 | TODO: 2차원 직사각형 합 문제 추가 | 포함-배제와 인덱스 기준 실수 줄이기 | 2D prefix sum |
