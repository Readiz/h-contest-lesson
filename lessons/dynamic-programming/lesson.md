...
| 단계 | 문제 | 목표 |
| --- | --- | --- |
| 입문 | 피보나치와 계단 오르기 | 상태, 초기값, 전이, 순서 네 가지 질문으로 1차원 DP 작성 |
| 표준 | 동전 개수와 0/1 배낭 | 무한 선택과 한 번 선택의 반복 방향 차이 확인 |
| 응용 | LIS와 구간 DP | `O(n^2)` DP에서 상태 의미와 분할 위치를 명확히 쓰기 |
| 함정 | 비트마스크 DP와 큰 2차원 DP | 상태 수와 메모리 사용량을 먼저 계산하고 overflow 방지 |
