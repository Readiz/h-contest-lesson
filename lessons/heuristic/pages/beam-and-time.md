...
`BEAM_SIZE`를 늘리면 더 많은 후보를 남기는 대신 복사·평가 횟수도 늘어납니다. 같은 시간 안에서 더 넓게 볼지, 더 깊게 볼지 비교해야 합니다.

### 두 단계로 폭을 줄이는 실전 예

다음 행동의 수가 매우 많으면 모든 행동으로 상태를 만든 뒤 beam을 고르는 것조차 비쌀 수 있습니다. [에어컨 설치 기사](/practice/AIRCONTECH) 사례에서는 각 상태에서 약 400개의 주택을 값싼 식으로 평가하되 상위 8개만 실제 상태로 만들고, 그렇게 만들어진 경로 전체에서 다시 상위 32개만 남깁니다.
...
1차 평가는 분기 수를 줄이고, 2차 평가는 서로 다른 경로의 미래 가능성을 비교합니다. 두 평가식은 목적이 다르므로 같은 식이나 같은 시간 패널티를 쓸 필요가 없습니다. 구체적인 상태와 평가식은 [31일 설치 일정 Beam Search](aircontech-beam-search.md)에서 살펴봅니다.
