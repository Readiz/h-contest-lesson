...
여기까지는 휴리스틱 풀이를 만드는 부품을 따로 보았습니다. 실제 문제에서는 이 부품들이 한 번에 섞입니다.

다음 페이지에서는 [광고판 도시 배치](/practice/BILLCITY)를 예시로, 아래 질문들이 한 풀이 안에서 어떻게 연결되는지 봅니다.

| 앞에서 본 질문 | 실전에서 확인할 것 |
| --- | --- |
| 답을 어떤 배열로 표현하는가? | 건물별 점유 상태와 광고별 배치 여부를 같이 든다 |
| 초기해는 어떻게 만드는가? | 점수와 건물 보너스를 보고 먼저 넣을 광고와 건물을 고른다 |
| 이웃 답은 어떻게 만드는가? | 이미 놓은 광고 여러 개를 지우고 다시 채운다 |
| 정확한 알고리즘은 어디에 섞는가? | 전체가 아니라 작은 건물이나 구역만 다시 탐색한다 |
| 실험 로그는 무엇을 비교하는가? | first-fit, 정렬 greedy, 위치 평가식, destroy/repair를 단계별로 비교한다 |
