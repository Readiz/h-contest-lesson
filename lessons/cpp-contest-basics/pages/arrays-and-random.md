...
`swap(a, a)`도 안전합니다. XOR 교환은 같은 원소를 넘겼을 때 값을 지울 수 있으므로 쓰지 않습니다. 배열 복사는 `O(n)`이며, 탐색 한 번마다 큰 상태를 복사하면 이 비용이 탐색 횟수를 제한합니다. 이후 레슨에서 변경 부분만 저장하고 되돌리는 방식으로 확장합니다.
...
- `copy`의 목적지·원본 순서가 맞는가?
- `Random`의 상태를 초기화했는가? `below(0)`을 호출하지 않는가?
- 셔플 범위에 고정해야 할 원소까지 넣지 않았는가?
- 같은 입력·같은 seed인데 결과가 다르면 미초기화 상태나 시간 종료의 영향을 확인했는가?
