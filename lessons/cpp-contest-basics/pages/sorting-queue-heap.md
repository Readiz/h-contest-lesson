...
매 독립 탐색 시작에 `clear()`를 호출합니다. `(key, id) = (5, 2), (1, 7), (1, 3)`을 넣으면 `(1, 3), (1, 7), (5, 2)` 순서로 나옵니다. push/pop은 `O(log size)`입니다. 빈 힙의 `pop`은 출력 인자를 바꾸지 않고 `false`를 반환하며, 용량을 넘는 `push`도 실패합니다. 큰 `Queue<CAP>`나 `MinHeap<CAP>` 인스턴스는 전역/정적으로 선언합니다.
