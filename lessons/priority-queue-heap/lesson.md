...
`priority_queue`는 중간 원소를 삭제하거나 값을 수정하는 기능이 없습니다. 원소는 복사해서 저장되므로 외부 배열 값을 바꿔도 힙의 순서는 갱신되지 않습니다. 특정 원소를 삭제해야 한다면 보통 지연 삭제를 씁니다.
...
헤더가 제한된 환경에서는 직접 구현합니다. 고정 용량의 완성 코드는 [공통 코드 레슨](https://h.readiz.com/learn/cpp-contest-basics/sorting-queue-heap)에 있습니다.
