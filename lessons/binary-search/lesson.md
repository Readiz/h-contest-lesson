...
| 단계 | 문제 | 목표 | 힌트 키워드 |
| --- | --- | --- | --- |
| 입문 | TODO: 정렬 배열에서 lower_bound 찾기 문제 추가 | 반열린 구간과 삽입 위치 이해 | `lower_bound` |
| 표준 | TODO: 최소 가능한 답 찾기 문제 추가 | 단조 판정 함수 만들기 | parametric search |
| 응용 | TODO: 최대 거리/최소 시간 최적화 문제 추가 | 답 범위와 판정 비용을 함께 계산 | `can(x)` |
| 함정 | TODO: 실수형 이분 탐색 문제 추가 | 반복 횟수와 오차 허용 범위 확인 | precision |
