...
## 로컬 연습: 그럴듯한 그리디의 작은 반례

회의 [start,end)를 가능한 많이 고릅니다. 시작 시각, 종료 시각, 입력 번호 순으로 정렬한 뒤 겹치지 않는 첫 회의부터 고르는 후보 알고리즘의 개수와 실제 최적 개수를 각각 구하세요.

**입력:** N 뒤 N줄의 start end. 1 <= N <= 20, 0 <= start < end <= 1000000입니다. 끝나는 시각과 다음 시작 시각이 같으면 함께 선택할 수 있습니다.

**출력:** 후보 알고리즘의 선택 개수와 최적 선택 개수를 출력합니다.

### 예시

```text exercise=proof-and-invariants role=input
3
0 10
1 2
2 3
...
```text exercise=proof-and-invariants role=output
1 2
```

**확인 방법:** N <= 20에서는 모든 subset의 양립 가능성을 검사해 최적 개수를 얻습니다. 예시에서 첫 회의는 나머지 두 회의를 모두 막습니다. 반례를 찾은 뒤 종료 시각 우선 규칙으로 바꾸고, 첫 선택을 최적해에 넣어도 개수가 줄지 않는 교환 논증을 문장으로 적습니다.
