...
## 2026-09-26 시각 자료와 실행 과정 재검토

등록 98개 레슨·168개 본문의 이미지 참조와 시각 설명 공백을 확인했습니다. 시작 시점에 29개 레슨에서 SVG 44개를 참조하고, 저장소에는 SVG 49개가 있었습니다. 이미지 존재 여부 대신 실제 입력·경계·상태 전이를 설명하는지, 모바일에서 글자가 읽히는지로 교체 대상을 골랐습니다. 알고리즘 본문 전체의 새로운 정당성 감사를 뜻하지는 않습니다.

| 대상 | 발견 사항과 수정 |
| --- | --- |
| 이분 탐색 | 정적 배열과 문장만으로 lo/mid/hi 이동을 설명하던 그림을 실제 lower_bound trace로 교체. 중복값·범위 밖 target 확인. |
| BFS | 큐와 발견 표시의 시간 관계를 직접 따라가도록 격자·큐·거리 데모 추가. 벽과 도달 불가 예제 제공. |
| Dijkstra | 후보와 dist의 차이를 단계별로 표시. 더 짧은 경로를 찾은 뒤 낡은 큐 항목을 버리는 순간 확인. |
| 2차원 누적합 | 좌표를 바꾸며 네 항의 영역과 중복 제거를 단계별로 비교. 선택 직사각형 합과 대조. |
| 열린 경로 2-opt | 실제 좌표를 공유하는 전후 SVG로 개선. 데모에서 구간 변경, 맨해튼 거리, 경계 차분과 전체 비용을 함께 표시. 끝점의 가짜 복귀 간선이 없음을 강조. |
| SA | current/best를 추이 그래프로 교체. 데모에서 온도·비용 증가량·채택 확률을 조작하고 같은 예제 난수로 current/best 보존을 확인. |
| destroy/repair·Beam | 배치 도형과 제거/재배치 과정을 표시. Beam의 잘못 연결된 화살표를 교정하고 실제 누적 score와 탐색 정렬용 key를 구분. |
| Fenwick·Segment Tree | Fenwick 담당 구간 및 쿼리 순서, lazy 태그 push 전후를 실제 구간/트리 도식으로 개선. |
| Radix Sort·배낭 DP | 실제 숫자의 두 pass와 안정 순서를 표시. DP는 정방향에서 같은 물건을 재사용하는 읽기/쓰기 셀을 역방향과 비교. |
| CCW·선분 교차 | 없던 그림 추가. 정수 좌표로 좌/우 회전, 교차·접촉·공선 조건을 본문과 연결. |

정적 SVG 8개 교체, 기하 SVG 1개 추가, 직접 조작 데모 6개를 반영합니다. 이분 탐색·BFS·Dijkstra·누적합의 기존 요약 그림은 본문에서 데모로 대체하며, 과거 Markdown 링크 호환을 위해 원본 asset 파일은 보존합니다. C++ 구현·manifest 구성은 바꾸지 않습니다.

데모 원본은 이 저장소의 `demos/`입니다. h-contest는 검증된 복사본을 같은 사이트에서 필요할 때만 로드합니다. 공개 Pages 독립 데모, 격리된 실제 앱 렌더링, 운영 응답/asset 배포는 각각 별도 검증 항목입니다.

검증: 생성 카탈로그 일치, 전체 98개 강의 validator와 기존 C++ 조합·경계·차분 48개(ASan/UBSan), 데모의 독립 기준값·중간 상태·경계·본문 링크 검사 47,097개 통과. 정적 그림 9개는 원본과 모바일 축소 렌더 및 수치를 검산했습니다. 운영과 같은 CSP를 적용한 격리 앱에서 6개 데모 × 1280px/390px 12조합의 이전·다음·재생·정지·입력 변경·키보드 진행·열기/닫기를 확인했고 가로 넘침과 브라우저 오류는 없었습니다. 실제 앱의 좁은 iframe 폭은 307px까지 확인했습니다. 주기적 앱 상태 갱신 때 열린 데모가 초기화되지 않고, 다른 탭으로 이동하면 재생이 멈추는 것도 검사했습니다. 인증은 fixture이며 실제 운영 로그인 세션 확인과는 구분합니다.

데모 코드·스타일은 열 때만 추가 로드합니다. 앱 기본/Markdown 청크의 gzip 합은 260,718→262,150바이트(+1,432), 데모를 연 뒤 정적 파일 4개의 gzip 합은 약18KB였습니다. 단계 이동 버튼은 변화하는 도식 위에 두어 설명 길이가 바뀔 때 클릭 위치가 이동하지 않게 했습니다.


## 2026-09-22 STL 없는 풀이 흐름과 공통 라이브러리 분리

기준 `3c4aa77`에서 제출·검증 입문과 재사용 코드를 분리했습니다. ORDERING 기준선 → 경로 개선 → 검증 → SA 순서와 해당 제출 코드는 유지하며, 공통 라이브러리를 모두 읽는 것을 선수 조건으로 두지 않습니다. 현재 98개 강의(기본 31·참고 67), 등록 본문 168개, C++ 블록 230개입니다.

| 대상 | 확인한 문제와 반영 |
| --- | --- |
| [실전 C++ 제출과 검증](lessons/cpp-contest-basics/lesson.md) | 제출 계약·TC 초기화와 ORDERING 실행을 두 하위 페이지로 연결. 자료구조 학습이 첫 풀이 진입을 가로막지 않도록 공통 구현은 별도 자료로 이동. |
| [STL 없는 공통 라이브러리](lessons/cpp-common-library/lesson.md) | 배열·난수·정렬·큐·힙 5개 독립 블록과 용량·초기화·실패 조건을 한곳에서 관리. 안정 정렬과 ID 동률 기준을 구분하고, 최소 힙에 삭제 없는 top 추가. |
| [정렬](lessons/sorting/lesson.md) | 비교 기준과 공통 정렬 구현 분리. 묶음 세기·Counting Sort를 호출 가능한 함수로 구성하고, ID를 통해 안정성을 확인. 기존 Radix Sort 코드 유지. |
| [그리디](lessons/greedy/lesson.md) | 강의실·과제 선택을 공통 정렬·힙과 조합하는 완성 함수로 변경. 방마다 마지막 종료 시각이 남는 힙의 의미, 입력 상한·작업 배열·64비트 합·TC 초기화를 명시. |
| [힙](lessons/priority-queue-heap/lesson.md) | STL 예제를 공통 최소 힙으로 교체하고 매 도착 직후 상위 K개 합을 기록. K=0·음수·중복·순간 적재량을 설명. 0-index 그림과 1-index 구현, 일반 heapify와 제공 API의 차이 구분. |
| 참조·작성 기준 | 다른 123개 본문은 공통 코드 링크만 새 자료로 변경. 일반 C++ 참고 환경 표시는 유지. CONTRIBUTING·ROADMAP과 생성 카탈로그에 분리 원칙·전환 범위를 반영. |

기존 배열·난수 및 정렬·큐·힙 Markdown 경로에는 이동 안내를 남겼으며, 등록 하위 페이지와 앱 내부 링크는 새 레슨으로 정리했습니다. 기존 앱의 두 하위 페이지 ID는 목록에서 제거합니다. 원본 Markdown의 이동 안내와 앱 route redirect는 서로 다른 기능입니다.

검증 결과:

- 전체 validator: 98개 강의, 기초 연습 19개, 공통·제출 코드 7개, ORDERING 탐색 검사, 기존 조합·경계·차분 사례 48개 통과. 생성 카탈로그 일치.
- [새 조합 검사](scripts/check_submission_examples.py): 제출 흐름 7개 레슨의 C++ 28개 블록에서 금지 의존성을 검사. 실제 정렬·그리디·힙 본문과 지정 공통 블록을 헤더 없이 컴파일한 뒤, 독립 기준 풀이와 ASan/UBSan으로 대조.
- 정렬: 동률·빈 입력·반복 호출, 안정 ID 정렬, 32비트 경계와 400만 원소 Radix Sort를 기준 정렬과 비교. 공통 정렬은 100만 ID와 구조체 안정성도 기존 공통 검사에서 확인.
- 그리디: 300개 무작위 사례에서 동시 강의 수와 모든 과제 부분집합으로 대조. 20만 항목·같은 종료/시작 시각·동일 마감·2×10^14 합·큰 TC → 작은 TC → 큰 TC 검사.
- 힙: 160개 스트림의 모든 K에서 각 prefix 정렬과 합 비교. 음수·중복·K=0/K=N·20만 항목·출력 경계와 반복 호출 검사.
- [선택 추출 도구](scripts/export_cpp_library.py): 본문을 원문으로 사용하며, 중복 선택 제거·헤더 없는 컴파일·잘못된 이름 거부·실패 시 기존 출력 보존 확인. 로컬 하네스의 STL은 제출 코드와 분리.
- 새 프론트엔드 빌드와 실제 lesson router·가짜 로그인 정보를 쓰는 격리 로컬 환경에서 PC 1280px·모바일 390px로 목차·본문·링크를 확인. 모바일 공통 블록 표는 두 열로 정리. 운영 인증 세션에서 본문을 직접 검증한 결과와 구분.

나머지 일반 C++ 예제를 전부 전환한 것은 아닙니다. 실제 풀이에 연결하는 단위마다 필요한 공통 기능과 실행 검증을 함께 추가합니다.
...
| 검토 항목 | 반영 |
| --- | --- |
| 점수 차이와 신뢰구간 | 보상 `B-A`·비용 `A-B`로 개선 방향을 통일. 양수 구간·음수 구간·0 포함을 구분하고 holdout 악화의 원인을 단정하지 않음. |
| BILLCITY 재탐색의 보장 | 방문 수 제한이 있는 DFS를 예산 제한 재탐색으로 표현. 후보 제한·안전한 가지치기·예산 종료의 보장을 구분. |
| Wavelet Matrix 값 범위 | 구현 입력은 `0..2^31-1`. 음수·64비트 정수는 좌표 압축이 필요하며 unsigned 변환만으로 순서를 보존하지 않음. |
| 코드 적용 환경 | C++가 있는 130개 문서에 일반 C++17·h-contest 공통/제출 코드·설계 조각을 표시. 공통 코드 허브에 자료구조별 전환 기준 추가. |
| 선수 관계와 표시 순서 | 같은 분류의 필수 선수가 먼저 오도록 기본 트랙 정렬. DP·고급 matching·Grundy 등 선택 참고 분리. validator에 역순·순환 의존 검사 추가. |
| 2-opt에서 SA로의 연결 | 비용 `57→65→53`의 실제 지역 최적 반례, STL 없는 실행 코드, 전체 차분·최선해·재현성 검사, 단계별 실측 추가. |
| 도입부의 구현 조건 | 74개 문서의 조건 문단을 관련 개념·코드 옆으로 이동하거나 주제별로 나눔. Matroid Intersection은 실제 교환으로 집합 크기가 늘어나는 예시로 보강. |
| 링크와 설명 범위 | Persistent 연습이 다루는 범위, AVL 회전과 Treap 선수 설명, sparse matvec 참조를 실제 내용에 맞춤. |
| 기본 강의 연습 공백 | 19개 로컬 연습에 입력·출력·제한·검산 방법을 추가. 기본 30개 모두 `linked`, 참고는 실제 상태대로 15개 `linked`·52개 `todo` 유지. |
| POMDP 식과 정확성 | 즉시 보상과 미래 보상 전체를 최대화하도록 괄호 수정. 정확 상태 표현과 belief 반올림 근사, 관측 정보와 숨은 상태를 구분. |

검증 결과:

- 전체 validator: 97개 강의의 구조·링크·생성 카탈로그, STL 없는 공통 코드 7개, 기존 조합·경계·차분 사례 41개 통과(ASan/UBSan).
- 새 기초 연습 19개: Markdown의 예제 입출력을 작은 독립 기준 풀이로 검산. 전체 입력 제한용 해법의 검증과는 구분.
- 새 SA: 6점 반례의 완전탐색, 무작위·경계 입력 200개에서 모든 2-opt 차분, 최선해 보존·같은 seed 재현성·0회 탐색·중복 좌표, 지수 근사 320001개 입력을 검사(ASan/UBSan).
- 선수 관계 검사: 정상 경로 통과, 역순과 분류 간 순환 관계 거부를 별도 확인.
- Matroid 교환 예시: 모든 부분집합으로 forest·색상 제약을 대조하고, 교환 경로와 두 가지 capacity의 최댓값 확인.
- 공개 ORDERING 채점기: 5개 방식 × `-O0`/`-O2`, 각 9회 실행 중 1회 워밍업 제외. 모든 경로 유효. 같은 후보 수에서 SA의 비용 합은 58368→56779, 전체 시간은 hill climbing 대비 약 1.9~2.0배. 같은 시간 비교나 운영 제출 결과로 해석하지 않음. [측정 원본](lessons/heuristic/lesson-assets/ordering-search-benchmark.json).

재현 스크립트는 [기초 연습 검산](scripts/check_foundation_exercises.py), [ORDERING 탐색 검증](scripts/check_heuristic_search.py), [기존 본문 조합 검증](scripts/check_review_examples.py)이며 모두 전체 validator에 포함됩니다. 작성 기준과 추가 문제 계획도 CONTRIBUTING·ROADMAP에 맞췄습니다.

화면 검증은 운영 프론트엔드 빌드와 실제 lesson router, 가짜 로그인 정보를 쓰는 격리 로컬 환경에서 진행했습니다. SA 반례·측정표, 기초 연습, POMDP 식, 예산 제한 재탐색의 5개 구간을 1280px·390px에서 확인했습니다. 모바일에서 열 정렬이 무너진 SA 좌표 텍스트는 표로, 긴 이동 trace는 단계 목록으로 바꿨습니다. 운영 로그인 화면에서 본문을 확인한 결과는 아닙니다.

## 2026-09-09 공개 탐색 지도 수정

공개 Pages를 직접 연 후 학습 지도 두 곳에서 삭제된 quadrangle-inequality-proofs ID가 일반 텍스트로 남은 것을 발견했습니다. Monge와 SMAWK로 연결하고, Markdown/HTML 생성기가 존재하지 않는 ID를 만나면 실패하도록 보완했습니다. 본문 검토와 별개로 생성된 탐색 지도까지 확인해야 하는 누락이었습니다.

## 유지 판단 재검토 — 2026-09-09

유지로 분류했던 기본자료 10개 강의의 본문·하위 페이지 18개를 다시 읽었습니다. 기존 그림의 존재 여부가 아니라 실제로 설명하는 상태 변화를 기준으로 재판단했습니다. 강의 97개·본문 169개는 유지합니다.

| 대상 | 재검토 결과 |
| --- | --- |
| C++ 기본기 (5개 본문) | TC 4→2→4 활성 구간 초기화 그림 추가. 나머지 배열·난수·자료구조·검증 계약은 유지. |
| 정렬 | 안정성·Radix 두 pass 설명과 기존 그림 유지. |
| 그리디 | 기존 사례 그림 유지. 검수 게이트 교환 증명에 최적 배정에 A가 없는 경우와 현재 시각이 빈 경우 보완. |
| 휴리스틱 (5개 본문) | 열린 경로 꼬리 2-opt, SA current/best, 두 광고를 함께 제거하는 repair 그림 3개 추가. 같은 비용의 SA 이동도 채택한다고 명시. AIRCONTECH 설명 유지. |
| TSP | 상태·경로·복원·대칭 거리 전제 재확인. 기존 그림 유지. |
| Union-Find | 대표 연결·크기·경로 압축·중복 union 설명 유지. |
| 제곱근 분할 | 기존 그림은 블록 배치·구간 질의용. lazy까지 설명한다는 이전 판단 정정, 저장값과 실제 값 그림 추가. |
| Hungarian | 두 비용 행렬·potential·교대 경로·직사각형 구현 전제 재확인, 기존 그림 유지. |
| Testing | 열린 경로 비용16→9를 사이클 차분식이0으로 계산하는 구체적 실패 입력 추가. |
| Proof | DP 경로를 마지막 이동 방향으로 분할하는 그림 추가. |

시각화 6개 추가는 서로 다른 상태 변화·반례를 설명하며, 본문 바로 옆에 배치합니다. 이전의 시각화 우선순위 표는 당시 판단 기록이고 이 절이 최신 재검토 결과입니다.

검증: 전체 validator(97개 강의), 공통 C++ 블록 7개와 조합·경계·차분 검사 41개 통과. 새 그림 6개는 실제 앱의 격리 로컬 화면에서 PC 1280px·모바일 390px로 각각 확인하고, 그림·주변 본문·확대 링크의 렌더링을 검사했습니다. 운영 로그인 본문을 확인한 결과는 아닙니다.
